defmodule Mix.Tasks.Discover do
  ...
  def run(args) do
    ...
    source =
      cond do
        opts[:base_url] && opts[:domains] ->
          AONCrawler.Crawler.Source.from_cli(
            id: :custom,
            name: opts[:name] || "Custom Source",
            base_url: opts[:base_url],
            allowed_domains: AONCrawler.Crawler.Source.parse_domains(opts[:domains]),
        ...
        opts[:source] ->
          ...
          case AONCrawler.Crawler.Source.get_source(source_id) do
            nil ->
              IO.puts(:stderr, "Unknown source: #{source_id}")
              IO.puts("Available sources: #{inspect(AONCrawler.Crawler.Source.source_ids())}")
              exit({:shutdown, 1})
        ...
        true ->
          ...
          case source_ids do
            ...
            [first | _] ->
              AONCrawler.Crawler.Source.get_source(first)
          end
      end

    max_concurrent =
      Application.get_env(:aoncrawler, :max_concurrent) ||
    ...
    if source do
      ...
      {:ok, urls} =
        AONCrawler.Crawler.Discovery.discover_urls_parallel(source, max_concurrent,
          max_urls: max_urls,
          max_depth: max_depth
        )
  ...
  defp discover_all_sources(max_concurrent, opts) do
    ...
    results =
      Enum.reduce(source_ids, %{}, fn source_id, acc ->
        source = AONCrawler.Crawler.Source.get_source(source_id)
        IO.puts("Source: #{source.name}")

        {:ok, urls} =
          AONCrawler.Crawler.Discovery.discover_urls_parallel(source, max_concurrent, opts)
