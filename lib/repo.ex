defmodule AONCrawler.Repo do
  @moduledoc """
  Ecto repository for AONCrawler's PostgreSQL database.
  ...
  @type document :: %AONCrawler.Document{}

  @typedoc "A chunk record from the database"
  @type chunk :: %AONCrawler.Chunk{}

  @typedoc "An embedding record from the database"
  @type embedding :: %AONCrawler.Embedding{}

  @typedoc "A crawl job record from the database"
  @type crawl_job :: %AONCrawler.CrawlJob{}
  ...
  def prefix, do: Application.get_env(:aoncrawler, :database_prefix)
  ...
  def unembedded_chunks(opts \\ []) do
    ...
    AONCrawler.Chunk
    |> join(:left, [c], e in AONCrawler.Embedding, on: c.id == e.chunk_id)
  ...
  def documents_with_chunk_counts(opts \\ []) do
    ...
    AONCrawler.Document
    |> join(:left, [d], c in AONCrawler.Chunk, on: d.id == c.document_id)
    |> group_by([d], d.id)
    |> select([d, c], %{
      document: d,
