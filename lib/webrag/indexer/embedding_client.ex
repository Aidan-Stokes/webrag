defmodule WebRAG.Indexer.EmbeddingClient do
  @moduledoc """
  ...
  defp do_embed_batch_ollama(texts, model, _batch_size) do
    results =
      Enum.map(texts, fn text ->
        case WebRAG.LLM.Ollama.embed(text, model: model) do
  ...
  defp openai_client do
    case Application.get_env(:webrag, :openai_client) do
      nil ->
        if Application.get_env(:webrag, :use_ollama, true) do
          WebRAG.LLM.Ollama
        end
