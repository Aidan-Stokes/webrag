defmodule WebRAG.Indexer.BatchProcessor do
  @moduledoc """
  ...
  Configure via `config.exs`:
      ...
      config :webrag, WebRAG.Indexer.BatchProcessor,
        batch_size: 100,
        batch_timeout: 5000,
  ...
  def init(opts) do
    ...
    if :ets.info(:webrag_batch_checkpoint) == :undefined do
      :ets.new(:webrag_batch_checkpoint, [:set, :named_table, :public])
    end
  ...
  defp process_batch(in_flight) do
    Task.Supervisor.async(WebRAG.Indexer.TaskSupervisor, fn ->
  ...
  defp insert_embedding(item, vector) do
    model =
      Application.get_env(
        :webrag,
        [WebRAG.Indexer, :embedding_model],
        "mxbai-embed-large"
      )
  ...
  defp save_checkpoint(state) do
    ...
    :ets.insert(:webrag_batch_checkpoint, {:checkpoint, checkpoint_data})
  end
